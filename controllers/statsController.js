...
exports.adminStats = catchAsyncErrors(async (req, res, next) => {
  ...
  const activeBids = await Product.find({
    status: "Approved",
    bidStatus: "Live",
  }).countDocuments();
  const endedBids = await Product.find({
    status: "Approved",
    bidStatus: "Expired",
  }).countDocuments();
  ...
  const totalVehicles = await Product.find({
    status: "Approved",
    category: "Vehicles",
  }).countDocuments();
  const totalProperties = await Product.find({
    status: "Approved",
    category: "Property",
  }).countDocuments();
  const totalMiscProducts = await Product.find({
    status: "Approved",
